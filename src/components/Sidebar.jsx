...
export default function Sidebar({
  ...
  const filteredConversations = conversations.filter((conv) => {
    ...
    if (conv.messages && conv.messages.length > 0) {
      const foundInContent = conv.messages.some(
  ...
  useEffect(() => {
    if (conversations.length > 0) {
      ...
      if (
        ...
        currentFirstId !== lastFirstConversationId
      ) {
        setCurrentPage(1);
      }
  ...
  const highlightSearchTerm = (text, searchTerm) => {
    ...
    return text.replace(
      regex,
      '<mark style="background: #FEF08A; color: #92400E;">$1</mark>'
    );
  };
              ...
              <div ref={trashMenuRef} className="trash-menu">
                ...
                <button
                  onClick={(e) => {
                    ...
                    if (
                      window.confirm(
                        `휴지통의 모든 대화(${trashCount}개)를 영구적으로 삭제하시겠습니까?\n\n이 작업은 되돌릴 수 없습니다.`
                      )
                    ) {
