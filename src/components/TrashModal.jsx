...
import { MAX_CONVERSATIONS } from "../entities/conversation/storage";
...
export default function TrashModal({
  ...
  const handleRestore = async (conversationId) => {
    ...
    setLoading(true);
    try {
      const restoredConversation = restoreFromTrash(
        conversationId,
        user.username
      );
  ...
  const handleClearTrash = () => {
    ...
    if (
      window.confirm(
        `휴지통의 모든 대화(${trashConversations.length}개)를 영구적으로 삭제하시겠습니까?\n\n이 작업은 되돌릴 수 없습니다.`
      )
    ) {
            ...
            <div className="trash-list">
              ...
              {trashConversations.map((conv) => (
                <div key={conv.id} className="trash-list-item">
                  ...
                  <div className="trash-date">{formatDate(conv.deletedAt)}</div>
